...
> ([`docs/3.4-aboutme-home-section.md`](3.4-aboutme-home-section.md), do
...
Aktualizacja modeli z `docs/3.4-aboutme-home-section.md` pkt. 4 — pola
...
3. [ ] Testy `tools` — odłożone na następny krok (ustalone), obecny pakiet
   zakłada stary schemat i nie przejdzie, dopóki nie zostanie przepisany
4. [ ] `aboutme` — budowane od razu z `JSONB` na `job_title`/`body_markdown`
5. [ ] `work` — budowane od razu z `JSONB` na `title`/`location`/`body_markdown`
6. [ ] `cv` — bez zmian, `CurriculumVitae` nie ma tłumaczalnych pól
...
`tools` przepisane na JSONB (kod, bez testów — te następne). `aboutme`/
`work`/`cv` czekają na sygnał do startu, budowane od razu z `JSONB`
zgodnie z tym planem, bez oddzielnego retrofitu.
