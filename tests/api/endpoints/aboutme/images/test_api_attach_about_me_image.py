...
from api.endpoints.urls import ADMIN_ABOUT_ME_IMAGE_ATTACH, ADMIN_FILE_CONFIRM, ADMIN_FILE_INIT, ADMIN_FILE_UPLOAD
...
def _confirmed_file_id(client, headers, *, directory: str = "aboutme") -> str:
...
class TestApiAdminAttachAboutMeImage:
    def test_attach01_returns_200_with_image_in_response(self, db_session):
        ...
        response = client.post(ADMIN_ABOUT_ME_IMAGE_ATTACH, json={"file_id": file_id}, headers=headers)

        assert response.status_code == 200
        images = response.json()["data"]["images"]
        assert len(images) == 1
        assert images[0]["file_id"] == file_id
        assert images[0]["url"].startswith("/static/aboutme/")

    def test_attach02_pending_file_returns_400(self, db_session):
        client = make_client(db_session, admin_aboutme_image_attach_router)
        headers = admin_auth_headers(db_session)
        create_test_about_me(db_session)
        file = create_test_file(db_session)

        response = client.post(ADMIN_ABOUT_ME_IMAGE_ATTACH, json={"file_id": str(file.id)}, headers=headers)

        assert response.status_code == 400
        assert response.json()["data"]["key_type_error"] == "InvalidStatus"

    def test_attach03_nonexistent_file_returns_404(self, db_session):
        client = make_client(db_session, admin_aboutme_image_attach_router)
        headers = admin_auth_headers(db_session)
        create_test_about_me(db_session)

        response = client.post(
            ADMIN_ABOUT_ME_IMAGE_ATTACH, json={"file_id": str(uuid.uuid4())}, headers=headers
        )

        assert response.status_code == 404

    def test_attach04_not_seeded_returns_404(self, db_session):
        client = make_client(db_session, admin_aboutme_image_attach_router)
        headers = admin_auth_headers(db_session)
    ...
    def test_attach05_already_attached_returns_409(self, db_session):
        ...
        create_test_about_me(db_session)
        file_id = _confirmed_file_id(client, headers)
        client.post(ADMIN_ABOUT_ME_IMAGE_ATTACH, json={"file_id": file_id}, headers=headers)
    ...
    def test_attach06_unauthenticated_returns_401(self, db_session):
        ...
        response = client.post(ADMIN_ABOUT_ME_IMAGE_ATTACH, json={"file_id": str(uuid.uuid4())})
