...
def _attach_new_confirmed_file(client, headers) -> str:
...
class TestApiAdminDetachAboutMeImage:
    def test_detach01_returns_200_and_removes_from_response(self, db_session):
        ...
        file_id = _attach_new_confirmed_file(client, headers)

        response = client.delete(ADMIN_ABOUT_ME_IMAGE_DETACH.format(file_id=file_id), headers=headers)
    ...
    def test_detach02_deletes_file_row_and_disk_bytes(self, db_session, static_root):
        ...
        client.delete(ADMIN_ABOUT_ME_IMAGE_DETACH.format(file_id=file_id), headers=headers)
    ...
    def test_detach03_not_attached_returns_404(self, db_session):
        client = make_client(db_session, admin_aboutme_image_detach_router)
        headers = admin_auth_headers(db_session)
        create_test_about_me(db_session)

        response = client.delete(ADMIN_ABOUT_ME_IMAGE_DETACH.format(file_id=uuid.uuid4()), headers=headers)

        assert response.status_code == 404

    def test_detach04_unauthenticated_returns_401(self, db_session):
        client = make_client(db_session, admin_aboutme_image_detach_router)
        create_test_about_me(db_session)

        response = client.delete(ADMIN_ABOUT_ME_IMAGE_DETACH.format(file_id=uuid.uuid4()))

        assert response.status_code == 401

    def test_detach05_non_admin_returns_403(self, db_session):
        client = make_client(db_session, admin_aboutme_image_detach_router)
        create_test_about_me(db_session)
        ...
        response = client.delete(
            ADMIN_ABOUT_ME_IMAGE_DETACH.format(file_id=uuid.uuid4()),
