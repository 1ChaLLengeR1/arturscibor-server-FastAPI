...
class TestApiAdminDeleteTool:
    def test_delete01_returns_200(self, db_session):
        client = make_client(db_session, admin_tools_delete_router)
        tool = create_test_tool(db_session)

        response = client.delete(ADMIN_TOOLS_DELETE.format(tool_id=tool.id), headers=admin_auth_headers(db_session))
    ...
    def test_delete02_row_removed_from_db(self, db_session):
        ...
        client.delete(ADMIN_TOOLS_DELETE.format(tool_id=tool.id), headers=admin_auth_headers(db_session))
    ...
    def test_delete03_removes_attached_files_from_disk(self, db_session, static_root):
        ...
        response = client.delete(ADMIN_TOOLS_DELETE.format(tool_id=tool.id), headers=headers)
    ...
    def test_delete04_nonexistent_returns_404(self, db_session):
        ...
        response = client.delete(
            ADMIN_TOOLS_DELETE.format(tool_id=uuid.uuid4()), headers=admin_auth_headers(db_session)
        )
    ...
    def test_delete05_unauthenticated_returns_401(self, db_session):
        ...
        response = client.delete(ADMIN_TOOLS_DELETE.format(tool_id=tool.id))
    ...
    def test_delete06_non_admin_returns_403(self, db_session):
        ...
        response = client.delete(
            ADMIN_TOOLS_DELETE.format(tool_id=tool.id), headers={"Authorization": f"Bearer {token}"}
        )
