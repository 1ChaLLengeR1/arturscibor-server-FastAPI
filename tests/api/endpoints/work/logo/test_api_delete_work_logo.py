...
from api.endpoints.admin.work.logo.delete import router as admin_work_logo_delete_router
...
def _set_confirmed_logo(client, headers, work_id) -> str:
    ...
    client.put(ADMIN_WORK_LOGO.format(work_id=work_id), json={"file_id": file_id}, headers=headers)
    return file_id


class TestApiAdminDeleteWorkLogo:
    def test_delete01_returns_200_and_clears_logo(self, db_session):
        client = make_client(
            db_session,
            ...
            admin_work_logo_delete_router,
        )
        headers = admin_auth_headers(db_session)
        work = create_test_work(db_session)
        _set_confirmed_logo(client, headers, work.id)

        response = client.delete(ADMIN_WORK_LOGO.format(work_id=work.id), headers=headers)

        assert response.status_code == 200
        assert response.json()["data"]["logo_file_id"] is None

    def test_delete02_removes_bytes_from_disk(self, db_session, static_root):
        client = make_client(
            db_session,
            admin_file_init_router,
            admin_file_upload_router,
        ...
        client.delete(ADMIN_WORK_LOGO.format(work_id=work.id), headers=headers)

        assert not saved.exists()

    def test_delete03_no_logo_returns_404(self, db_session):
        client = make_client(db_session, admin_work_logo_delete_router)
        headers = admin_auth_headers(db_session)
        work = create_test_work(db_session)
    ...
    def test_delete04_unauthenticated_returns_401(self, db_session):
        ...
        response = client.delete(ADMIN_WORK_LOGO.format(work_id=work.id))
    ...
    def test_delete05_non_admin_returns_403(self, db_session):
        ...
        response = client.delete(
            ADMIN_WORK_LOGO.format(work_id=work.id), headers={"Authorization": f"Bearer {token}"}
        )
