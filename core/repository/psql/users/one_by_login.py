...
def one_by_login_psql(
    login: str, db_session: Session | None = None
) -> tuple[UserResponse | None, ApiErrorData | None, bool]:
    try:
        with managed_session(db_session) as (db, _):
            user = db.execute(select(Users).where(Users.login == login)).scalar_one_or_none()
            if user is None:
                return (
                    None,
                    ApiErrorData(
                        message="User not found",
                        type_module="one_by_login_psql",
