...
from api.endpoints.urls import ADMIN_TOOLS_IMAGE_ATTACH
...
from api.schemas.tools.image import ToolImageAttachPayload
from api.schemas.tools.response import ToolResponseData
from core.handler.tools.images.attach import handler_attach_tool_image
from database.psql.database import get_db

router = APIRouter()

_STATUS_BY_KEY = {"NotFound": 404, "AlreadyAttached": 409}


@router.post(
    ADMIN_TOOLS_IMAGE_ATTACH,
    summary="[Admin] Attach a confirmed file to a tool",
    ...
    responses={
        ...
        404: {"model": ApiErrorResponse, "description": "Tool or file not found"},
        409: {"model": ApiErrorResponse, "description": "File already attached to a tool"},
        500: {"model": ApiErrorResponse, "description": "Unexpected server error"},
    },
    status_code=200,
    tags=["Admin/Tools"],
)
def api_admin_attach_tool_image(
    tool_id: str,
    body: ToolImageAttachPayload,
    db: Session = Depends(get_db),
    _current_user: dict = Depends(JWTAuthenticationMiddleware(roles=["admin"])),
) -> ApiResponse[ToolResponseData, None] | JSONResponse:
    try:
        result, error, ok = handler_attach_tool_image(tool_id, body.file_id, db_session=db)
        if not ok:
        ...
        error = ApiErrorData(
            message=str(e),
            type_module="api_admin_attach_tool_image",
