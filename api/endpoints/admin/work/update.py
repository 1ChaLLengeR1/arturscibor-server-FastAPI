...
from api.schemas.work.update import WorkUpdatePayload
from core.handler.work.update import handler_update_work
...
def api_admin_update_work(
    work_id: str,
    body: WorkUpdatePayload,
    db: Session = Depends(get_db),
    _current_user: dict = Depends(JWTAuthenticationMiddleware(roles=["admin"])),
) -> ApiResponse[WorkResponseData, None] | JSONResponse:
    try:
        result, error, ok = handler_update_work(work_id, db_session=db, **body.model_dump(exclude_unset=True))
        if not ok:
        ...
        error = ApiErrorData(
            message=str(e), type_module="api_admin_update_work", type_error="exception", key_type_error="Exception"
        )
