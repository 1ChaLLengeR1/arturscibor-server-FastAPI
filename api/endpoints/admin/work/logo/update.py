...
from api.schemas.work.logo import WorkLogoAttachPayload
from api.schemas.work.response import WorkResponseData
from api.status import STATUS_BY_KEY
from core.handler.work.logo.update import handler_update_work_logo
...
@router.put(
    ADMIN_WORK_LOGO,
    summary="[Admin] Replace a company's logo",
    ...
    responses={
        ...
        404: {"model": ApiErrorResponse, "description": "Work or file not found"},
        500: {"model": ApiErrorResponse, "description": "Unexpected server error"},
    },
    status_code=200,
    tags=["Admin/Work"],
)
def api_admin_update_work_logo(
    work_id: str,
    body: WorkLogoAttachPayload,
    db: Session = Depends(get_db),
    _current_user: dict = Depends(JWTAuthenticationMiddleware(roles=["admin"])),
) -> ApiResponse[WorkResponseData, None] | JSONResponse:
    try:
        result, error, ok = handler_update_work_logo(work_id, body.file_id, db_session=db)
        if not ok:
        ...
        error = ApiErrorData(
            message=str(e),
            type_module="api_admin_update_work_logo",
