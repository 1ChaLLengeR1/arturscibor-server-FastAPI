#!/usr/bin/env bash
# Usage: source infra/scripts/load_env.sh <local|prd>
# Loads env/<local|prd>.env into the current shell and sanity-checks the
# required ARTURSCIBOR_BACKEND_DB_* variables are present.
set -euo pipefail

ENV_NAME="${1:?Usage: load_env.sh <local|prd>}"
